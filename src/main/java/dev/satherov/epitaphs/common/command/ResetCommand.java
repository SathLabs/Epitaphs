...
public class ResetCommand {
    ...
    ///
    /// Hard resets a player to a given timestamp. *time travel*
    ///
                ...
                .then(Commands.literal("player")
                        .then(Commands.argument("player", EntityArgument.player())
                                .then(Commands.argument("timestamp", StringArgumentType.string())
                                        ...
                                        .executes(ctx -> ResetCommand.execute(ctx, EntityArgument.getPlayer(ctx, "player").getUUID()))
                                )
                        )
                )
                .then(Commands.literal("uuid")
                        .then(Commands.argument("uuid", UuidArgument.uuid())
                                ...
                                .then(Commands.argument("timestamp", StringArgumentType.string())
                                        ...
                                        .executes(ctx -> ResetCommand.execute(ctx, UuidArgument.getUuid(ctx, "uuid")))
                                )
                        )
                );
        // @formatter:on
    ...
    private static int execute(CommandContext<CommandSourceStack> ctx, UUID uuid) {
        ...
        if (result < 1) {
            source.sendFailure(EPCommandLang.COMMAND_RESET_FAILURE.translate(EPCommands.formatPlayer(profile), timestamp).style(ChatFormatting.RED));
            return 0;
        } else {
            source.sendSuccess(() -> EPCommandLang.COMMAND_RESET_SUCCESS.translate(EPCommands.formatPlayer(profile), timestamp).style(ChatFormatting.GREEN), true);
            return 1;
        }
    }
}
