...
public class ResetCommand {
    ...
    private static int execute(CommandContext<CommandSourceStack> ctx, UUID uuid) {
        ...
        if (result < 1) {
            source.sendFailure(EPLanguage.COMMAND_RESET_FAILURE.text(EPCommands.formatPlayer(profile), timestamp).withStyle(ChatFormatting.RED));
            return 0;
        } else {
            source.sendSuccess(() -> EPLanguage.COMMAND_RESET_SUCCESS.text(EPCommands.formatPlayer(profile), timestamp).withStyle(ChatFormatting.GREEN), true);
            return 1;
        }
    }
}
